...
fn main() {
    ...
    let (recursive_data, proof_target, verifier_data_target) = recursive_verifier_circuit(&base_data);
    ...
    recursive2_witness.set_proof_with_pis_target(&proof_target, &base2_proof);
    recursive2_witness.set_verifier_data_target(&verifier_data_target, &base2_data.verifier_only);

    let  recursive2_proof = recursive_data.prove(recursive2_witness).unwrap();

    // recursive2_data.common = recursive_data.common;
    recursive_data.verify(recursive2_proof).unwrap();
...
fn recursive_verifier_circuit(base_data: &CircuitData<F, C, 2>) -> (CircuitData<F, C, 2>, ProofWithPublicInputsTarget<2>, VerifierCircuitTarget) {
    ...
    (builder.build::<C>(), proof, verifier_data)
